...
//Pare Add Woocommerce order custom field "Vendor paypal account"
//
/**
 * Adds 'order_total' column header to 'Orders' page immediately after 'Total' column.
 *
...
function sv_wc_cogs_add_order_profit_column_header( $columns ) {
    ...
    foreach ( $columns as $column_name => $column_info ) {
        ...
        if ( 'order_total' === $column_name ) {
            $new_columns['paid_date'] = __( 'Paid Date', 'my-textdomain' );
			$new_columns['paypal'] = __( 'Paypal', 'my-textdomain' );
        }
    }
...
/**
 * Pare
 * Adds 'Paypal' column content to 'Orders' page immediately after 'Total' column.
 *
 * @param string[] $column name of column being displayed
 */
function add_order_paypal_column_content( $column ) {
    global $post;
    ...
    if ( 'paypal' === $column ) {
        ...
        $paypal   = do_shortcode('[wpuf-meta name="paypal_email_address"]');//'xxx@gmail.com';
     

          
       
echo 'xxx';
      	echo array_shift(get_post_meta( $post->ID, 'paypal_email_address', true ));
		echo array_shift(get_post_meta( $post->ID, 'paid_date', true ));
		
    }
}
add_action( 'manage_shop_order_posts_custom_column', 'add_order_paypal_column_content' );
//Pare Add Disable Plugin Update
